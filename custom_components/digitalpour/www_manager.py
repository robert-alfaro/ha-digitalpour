"""Manager for shipping and auto-registering the bundled DigitalPour card."""
...
async def async_setup_cards(hass: HomeAssistant) -> bool:
    ...
    try:
        ...
        _LOGGER.error("Failed to install DigitalPour card asset: %s", err)
        return False

    _LOGGER.debug("Installed bundled DigitalPour card to %s", target)
    return True


async def async_register_cards(hass: HomeAssistant) -> None:
    """Ensure the bundled DigitalPour card resource exists in Lovelace."""
